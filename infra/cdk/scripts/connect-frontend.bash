...
AWS_ECS_FRONTEND_SERVICE_NAME=$(jq -r '.AWS_ECS_FRONTEND_SERVICE_NAME' <<< "$ENV_CONTENT")
...
TASK_ID=$(aws ecs list-tasks --cluster "$AWS_ECS_CLUSTER_NAME" --region "$AWS_REGION" --profile "$AWS_ACCOUNT_ID" --output json --service "$AWS_ECS_FRONTEND_SERVICE_NAME" | jq '.taskArns[0]' | sed 's/.*\///g' | sed 's/"//g')
...
aws ecs execute-command --cluster "$AWS_ECS_CLUSTER_NAME" --region "$AWS_REGION" --service-name "$AWS_ECS_FRONTEND_SERVICE_NAME" --profile "$AWS_ACCOUNT_ID" --interactive --command "/bin/sh" --task "$TASK_ID"
