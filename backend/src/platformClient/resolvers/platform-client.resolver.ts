...
import { applicationConfig, inviteEmailTemplates } from 'src/config';
...
export class PlatformClientResolver {
  ...
  async sendUserInvites(
    ...
    try {
      const inviteMailPayloads = userInvites.userInvites?.map((userInvite) => ({
        to: userInvite.email,
        from: 'noreply@truck-os.de',
        templateId: inviteEmailTemplates[locale],
        dynamicTemplateData: {
          user: {
      ...
      const sendInviteEmailPromises = inviteMailPayloads.map((payload) => this.sendGridClient.send(payload));
      await Promise.all(sendInviteEmailPromises);
      return {
        success: userInvites.userInvites,
        errors: [],
      };
    } catch (err) {
      return {
